...
export interface Item {
  ...
  Image?: ImageWidget;
  ImageAlt?: string;
  /** @description Vídeo do YouTube diminui o PageSpeed (performance)  */
  ...
  const content = items && (
    <div>
      <h2 class="text-4xl font-semibold text-center">
        {items[selectedContent].Title}
      </h2>

      {items[selectedContent].chosenMidia == "Video" &&
        items[selectedContent].Video && (
            ...
            >
              ...
              <object data="" width="320" height="240">
                <embed
                  width="320"
                  height="240"
                  src={items[selectedContent].Video}
                />
              </object>
            </video>
          </div>
      ...
      {items[selectedContent].chosenMidia == "Youtube video" &&
        items[selectedContent].YoutubeVideo && (
          <div class="h-[60vw] w-[100vw] md:w-[50vw] md:h-[35vw]  xl:w-[1000px] xl:h-[600px]">
            <iframe
              class="w-full h-full"
              width="1000"
              height="1000"
      ...
      {items[selectedContent].chosenMidia == "Image" &&
        items[selectedContent].Image && (
      ...
      {items && selectedContent != items.length - 1 && (
        <div class="flex justify-end">
          <button
            ...
            hx-get={useSection({
              props: {
                selectedContent: selectedContent + 1,
                renderContent: true,
              },
            })}
            hx-target="#content"
          >
            <span class="text-xl text-secondary pt-2 mr-2">
              {items[selectedContent + 1].Title}
            </span>
        ...
        <ul class="bg-primary-content min-w-[360px] rounded-xl p-4 flex md:flex-col overflow-x-scroll md:overflow-x-hidden">
          {items &&
            items.map((item, index) => (
              <li
                class={`text-xl p-3 border-b last:border-0 border-primary min-w-64`}
              >
                <button
                  hx-trigger="click"
                  hx-get={useSection({
                    props: { selectedContent: index, renderContent: true },
                  })}
                  hx-target="#content"
